...
<dd>
<p>クイズの解答は各コンテンツの動画ごとにcsvファイルで管理しています。複数の解答のパターン（漢字表記・ひらがな表記、あるいは別解等）は可能な範囲でカバーするようにしてはおりますが、網羅はできていません。<br>明らかに正答なのに誤答になってしまった、という場合はご一報ください。内容に応じてcsvファイルの更新を検討させていただきます。</p>
</dd>
</dl>
<dl class="qa">
<dt>動画視聴ページが正しく表示されない（レイアウトがくずれる、早押しボタンの画像が表示されない等）。</dt>
<dd>
<p>ページの再読み込みを実施してください（クイズの正答数・誤答数は引き継がれません）。<br>ページを再度読み込んでも改善しない場合、使用している端末の情報（OS、ブラウザ）とともにご連絡ください。</p>
</dd>
</dl>
<dl class="qa">
<dt>シークバーを使用して一度飛ばした問題に、再度解答したい。</dt>
<dd>
<p>シークバーを使用して一部の問題を飛ばした場合、遡って再回答することはできません。すべての問題に解答したい場合は、スキップせずに動画を視聴ください。<br>初めから解きなおしたい場合、ページの再読み込みを実施してください。</p>
</dd>
</dl>
<dl class="qa">
<dt>コンテンツ一覧に並んでいる動画のサムネイル画像が表示されない</dt>
<dd>
